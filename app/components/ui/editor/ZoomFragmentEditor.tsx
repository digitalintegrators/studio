...
export function ZoomFragmentEditor({
    ...
    const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>, point: 'start' | 'end') => {
        ...
        const move = (ev: PointerEvent) => {
            ...
            if (point === 'start') {
                ...
                onUpdate({ movementEndX: x, movementEndY: y });
            }
        };
    ...
    const handlePreviewClick = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (movementEnabled && editingPoint === 'end') {
            onUpdate({ movementEndX: x, movementEndY: y });
        } else {
            onUpdate({ focusX: x, focusY: y });
        }
    ...
    const handleToggleMovement = () => {
        if (!movementEnabled) {
            ...
            onUpdate({
                ...
                movementEndOffset: holdDuration,
            });
        } else {
            onUpdate({
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-5">
                <div>
                    <div className="flex items-center gap-2 text-xs mb-2 text-white/60">
                        ...
                        <span>{movementEnabled ? "Puntos de enfoque" : "Punto de enfoque"}</span>
                        {movementEnabled && (
                            <div className="ml-auto flex gap-1">
                                <button
                                    onClick={() => setEditingPoint('start')}
                                    className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors ${editingPoint === 'start'
                                ...
                                <button
                                    ...
                                    className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors ${editingPoint === 'end'
                    ...
                    >
                        ...
                        <div className="absolute bottom-2 left-0 right-0 flex justify-center pointer-events-none">
                            <span className="text-[7px] text-white/20 font-mono uppercase tracking-[0.3em]">
                                {movementEnabled ? "Arrastra A y B" : "Arrastra o click"}
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-3 p-3 bg-white/3 border border-white/8 rounded-lg">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <p className="text-xs font-medium text-white/80">Movimiento de cámara</p>
                                <p className="text-[10px] text-white/40">Desplazamiento durante el zoom</p>
                            </div>
                        </div>
                        <button
                            onClick={handleToggleMovement}
                            className={`relative w-11 h-6 rounded-full transition-colors ${movementEnabled ? 'bg-emerald-500' : 'bg-white/15'
                                }`}
                        >
                    ...
                    {/* Movement Range Timeline (only when movement enabled) */}
                    {movementEnabled && (() => {
                        const holdDuration = calculateHoldDuration(fragment);
                            ...
                            const handleMove = (ev: MouseEvent) => {
                                ...
                                if (type === 'start') {
                                    ...
                                    onUpdate({
                                        ...
                                        movementEndOffset: Math.round(newEnd * 10) / 10
                                    });
                                }
                            };
                        ...
                        return holdDuration > 0.1 ? (
                            <div className="space-y-3 p-3 bg-[#0A0A0A] border border-[#262626] rounded-xl group transition-all">
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-2 text-[11px] font-medium text-white/50">
                                        <span>Timeline de Movimiento</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <span className="text-[10px] font-mono text-emerald-400/80 bg-emerald-500/5 px-1.5 py-0.5 rounded border border-emerald-500/10">
                                            {(endOffset - startOffset).toFixed(1)}s activo
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-3">
                                    <div className="relative h-9 flex rounded-md overflow-hidden bg-[#141414] border border-[#1F1F1F]">
                                        <div
                                            className="h-full bg-white/3 border-r border-white/5 flex items-center justify-center opacity-40 hover:opacity-100 transition-opacity"
                                        ...
                                        <div
                                            className="h-full relative flex-1 bg-transparent"
                                            style={{ width: `${holdPct}%` }}
                                            data-hold-timeline
                                        >
                                            <div
                                                ...
                                                onMouseDown={(e) => handleTimelineDrag(e, 'range')}
                                            >
                                                <div
                                        ...
                                        <div
                                            className="h-full bg-white/3 border-l border-white/5 flex items-center justify-center opacity-40 hover:opacity-100 transition-opacity"
                                            style={{ width: `${exitPct}%` }}
                                        >
                            ...
                            <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                                <div className="flex items-center gap-2 text-xs text-amber-400/80">
                                    <span>Fragmento muy corto para movimiento. Aumenta la duración o reduce la velocidad de transición.</span>
                                </div>
                            </div>
                        );
                    })()}
                ...
                {/* 3D Perspective Toggle and Controls */}
                <div className="space-y-3 p-3 bg-white/3 border border-white/8 rounded-lg">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <Icon icon="mdi:cube-outline" width="16" className="text-white/50" />
                            <div>
                                <p className="text-xs font-medium text-white/80">Efecto 3D</p>
                                <p className="text-[10px] text-white/40">Perspectiva con profundidad</p>
                            </div>
                        </div>
                        <button
                            onClick={handleToggle3D}
                            className={`relative w-11 h-6 rounded-full transition-colors ${fragment.enable3D ? 'bg-gray-400' : 'bg-white/15'
                                }`}
                        >
                            <div
                                className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${fragment.enable3D ? 'translate-x-6' : 'translate-x-1'
                                    }`}
                            />
                        </button>
                    </div>

                    {/* 3D Controls (only when enabled) */}
                    {fragment.enable3D && (
                        <div className="space-y-3 pt-3 border-t border-gray-500/20">
                            <SliderControl
                                icon="mdi:brightness-6"
                                label="Intensidad"
                                value={fragment.perspective3DIntensity ?? 50}
                                min={0}
                                max={100}
                                step={5}
                                onChange={(value) => onUpdate({ perspective3DIntensity: value })}
                                suffix="%"
                            />

                            <div className="space-y-2">
                                <div className="flex items-center gap-2 text-xs text-white/50">
                                    <span>Dirección de perspectiva</span>
                                </div>
                                ...
                                <div
                                    ...
                                    onClick={(e) => {
                                        ...
                                        onUpdate({
                                            perspective3DAngleX: Math.round(y * 45),
                                            perspective3DAngleY: Math.round(-x * 45)
                                        });
                                    }}
                                >
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <div
                                            className="w-19 h-13 border border-gray-500/50 bg-gray-500/10 rounded-sm transition-transform duration-300 ease-out"
                                            style={{
                                                transform: `perspective(60px) rotateX(${fragment.perspective3DAngleX ?? 0}deg) rotateY(${fragment.perspective3DAngleY ?? 0}deg)`
                                            }}
                                        />
                                    </div>

                                    <div
                                        className="absolute w-2.5 h-2.5 bg-gray-300 rounded-full shadow-[0_0_8px_rgba(255,255,255,0.2)] transition-all duration-300 ease-out z-10"
                                        style={{
                                ...
                                <p className="text-[10px] text-white/30 text-center">
                                    Click para elegir dirección de inclinación
                                </p>
                            </div>
                        </div>
                    )}
