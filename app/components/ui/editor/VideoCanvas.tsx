...
>(function VideoCanvas(props, ref) {
  ...
  const zoomTransform = useMemo(() => {
    ...
    const isMoving =
      activeZoomFragment.movementEnabled && phaseState.phase === "hold";
    const transitionMs = isMoving
      ? Math.max(120, Math.round(speedToTransitionMs(activeZoomFragment.speed) * 0.22))
                  ...
                  transition:
                    mediaType === "image" && apply3DToBackground
                      ...
                      : zoomTransform.isMoving
                        ? `transform ${Math.max(90, zoomTransform.transitionMs)}ms ${ZOOM_LINEAR_MOVEMENT_EASING}`
                        : `transform ${zoomTransform.transitionMs}ms ${ZOOM_EASING}`,
                  willChange: mediaType === "video" ? "transform" : undefined,
                }}
              >
                {" "}
                {mediaType === "image" && apply3DToBackground && (
