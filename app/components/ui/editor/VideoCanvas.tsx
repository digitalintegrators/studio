...
import {
  VIDEO_Z_INDEX,
  BOTTOM_ONLY_RADIUS_MOCKUPS,
  SELF_SHADOWING_MOCKUPS,
} from "@/lib/constants";
...
export const VideoCanvas = forwardRef<
  VideoCanvasHandle,
  ExtendedVideoCanvasProps
>(function VideoCanvas(props, ref) {
  const {
    mediaType = "video",
    imageUrl = null,
    imageRef,
  ...
  const zoomTransform = useMemo(() => {
    if (!activeZoomFragment) {
      ...
      const exitMs = lastFragment
        ? speedToTransitionMs(lastFragment.speed)
        : speedToTransitionMs(3);
      return {
        scale: 1,
        translateX: 0,
        translateY: 0,
    ...
    const cursorFollowFrame =
      activeZoomFragment.followCursor && cursorData?.hasCursorData
        ? interpolateCursorPosition(
            cursorData.keyframes,
            currentTime,
            Math.round((activeZoomFragment.followSmoothing ?? 0.62) * 100),
          )
        : null;
    ...
    const translateX = 50 - followedFocus.focusX;
    const translateY = 50 - followedFocus.focusY;
    const isMoving =
      (activeZoomFragment.movementEnabled || activeZoomFragment.followCursor) && phaseState.phase === "hold";
    const transitionMs = isMoving
      ? 50
      : speedToTransitionMs(activeZoomFragment.speed);
    return {
  ...
  }, [activeZoomFragment, zoomFragments, currentTime, cursorData]);
  ...
  const shouldShowWallpaper =
    backgroundTab === "wallpaper" &&
    selectedWallpaper >= 0 &&
    !shouldShowUnsplashOverride;
  const shouldShowCustomImage =
    backgroundTab === "image" && selectedImageUrl !== "";
  const shouldShowCustomColor =
    backgroundTab === "color" && !!backgroundColorCss;
  ...
  const exportDimensions = useMemo(() => {
    if (
      (aspectRatio === "auto" || aspectRatio === "custom") &&
      customAspectRatio
    ) {
      return {
  ...
  const preservedVideoStateRef = useRef<{
    time: number;
    playing: boolean;
  } | null>(null);
  useEffect(() => {
  ...
  useEffect(() => {
    if (videoRef.current && videoUrl) {
      ...
      const needsSrc =
        !videoSrc || videoSrc === "" || videoSrc === window.location.href;
  ...
  const [videoHoverCorner, setVideoHoverCorner] = useState<Corner | null>(
    "top-right",
  );
  ...
  const [isDraggingElementRotation, setIsDraggingElementRotation] =
    useState(false);
  ...
  const multiDragStartRef = useRef<Map<string, { x: number; y: number }>>(
    new Map(),
  );
  ...
  useEffect(() => {
    if (!isDraggingMask || !onUpdateMaskFragment) return;
  ...
  }, [isDraggingMask, onUpdateMaskFragment]);
  ...
  const handleMaskPointerDown = useCallback(
    (
      ...
      if (!onUpdateMaskFragment) return;

      event.preventDefault();
      event.stopPropagation();

      onSelectMaskFragment?.(fragment.id);
      setIsVideoSelected(false);
      onElementSelect?.(null);
    ...
    [onElementSelect, onSelectMaskFragment, onUpdateMaskFragment],
  );
  const maskStyles = useMemo(() => {
    ...
    if (config.top) {
      masks.push(
        `linear-gradient(180deg, transparent ${config.top.from}%, black ${config.top.to ?? 100}%)`,
      );
    }
    if (config.bottom) {
      masks.push(
        `linear-gradient(0deg, transparent ${config.bottom.from}%, black ${config.bottom.to ?? 100}%)`,
      );
    }
    if (config.left) {
      masks.push(
        `linear-gradient(90deg, transparent ${config.left.from}%, black ${config.left.to ?? 100}%)`,
      );
    }
    if (config.right) {
      masks.push(
        `linear-gradient(270deg, transparent ${config.right.from}%, black ${config.right.to ?? 100}%)`,
      );
    }
    if (config.angle !== undefined) {
      masks.push(
        `linear-gradient(${config.angle}deg, transparent ${config.angleFrom ?? 0}%, black ${config.angleTo ?? 100}%)`,
      );
    }
  ...
  const imageUrlToLoad = shouldShowCustomImage
    ? selectedImageUrl
    : shouldShowUnsplashOverride
      ? unsplashOverrideUrl
      : null;
    ...
    for (const element of canvasElements) {
      if (element.type === "image") {
        ...
        if (!cache.has(imageElement.imagePath)) {
          ...
          img.onerror = () => {
            console.error(
              `Failed to load canvas element image: ${imageElement.imagePath}`,
            );
          };
  ...
  useEffect(() => {
    ...
    const syncTime = () => {
      if (
        !camVideo.seeking &&
        Math.abs(camVideo.currentTime - mainVideo.currentTime) > 0.15
      ) {
        try {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (isDraggingElementRotation) {
        ...
        if (!wasDragRef.current && Math.abs(deltaX) < 3 && Math.abs(deltaY) < 3)
          return;
        ...
        if (canvasSelectedIds.length > 1) {
          ...
          let newX = Math.max(
            0,
            Math.min(100, elementDragStart.current.initialX + percentX),
          );
          let newY = Math.max(
            0,
            Math.min(100, elementDragStart.current.initialY + percentY),
          );
          const SNAP_THRESHOLD = 2;
          const centerX = 50;
          const centerY = 50;
  ...
  const drawCursorToCanvas = (
    ...
    const frame = getSafeCursorFrame(
      cursorData,
      frameTime,
      cursorConfig.smoothing,
    );
    ...
    if (cursorConfig.style === "dot") {
      ...
      ctx.strokeStyle =
        color.toLowerCase() === "#ffffff" ? "#111827" : "#ffffff";
      ctx.stroke();
    }
    ctx.restore();
  };
  const drawFrame = async () => {
    ...
    const zoomState = calculateSmoothZoom(frameTime, zoomFragments, cursorData);
    ...
    const drawMockupAndMedia = (
      ...
      if (applyImageXform && imageTransform && !apply3DToBackground) {
        if (
          imageTransform.perspective &&
          imageTransform.perspective > 0 &&
        ...
        c.scale(
          imageTransform.scale * imageZoomScale,
          imageTransform.scale * imageZoomScale,
        );
      ...
      let vR = scaledRadius;
      if (hasMockupLocal) {
        const mBlur = SELF_SHADOWING_MOCKUPS.includes(mockupId)
          ? scaledShadowBlur
          : 0;
        ...
        vH = mr.contentHeight;
        vR =
          mockupId === "iphone-slim" ||
          mockupId === "glass-curve" ||
          mockupId === "glass-full"
            ? scaledRadius * 6
            : scaledRadius;
      }
      c.save();
      const bottomOnly =
        hasMockupLocal && BOTTOM_ONLY_RADIUS_MOCKUPS.includes(mockupId);
      if (vR > 0) {
        if (bottomOnly) {
    ...
    const has3DEffect =
      zoomState.perspective > 0 &&
      (zoomState.rotateX !== 0 || zoomState.rotateY !== 0);
    ...
    const activeFragment =
      ...
      zoomFragments
        ...
        .sort((a, b) => b.endTime - a.endTime)[0];
    const targetScale = activeFragment
      ? zoomLevelToFactor(activeFragment.zoomLevel)
      : zoomState.scale;
    ...
    ctx.restore();
    const { containerX, containerY, containerWidth, containerHeight } =
      computeContainer();
  ...
  const drawCameraOverlay = async (
    ...
    if (mainVideo && camVideo.paused) {
      const targetTime = Math.min(
        ...
        Math.max(0, camVideo.duration - 0.1),
      );
    ...
    const {
      ...
      left: drawX,
      top: drawY,
    } = getCameraLayout(cameraConfig, canvasWidth, canvasHeight);
    if (size <= 0) return;
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              ...
              {!isMulti && singleId && (
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      const maxZ = Math.max(
                        ...canvasElements.map((e) => e.zIndex),
                        VIDEO_Z_INDEX,
                      );
                      if (onElementUpdate)
                        onElementUpdate(singleId, { zIndex: maxZ + 1 });
                      setCanvasCtxMenu(null);
                    }}
                  >
                    {" "}
                ...
                <>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      ids.forEach((id) => {
                        if (onElementUpdate)
                          onElementUpdate(id, { groupId: newGroupId });
                      });
                      setCanvasCtxMenu(null);
                    }}
                  >
                    ...
                    onClick={() => {
                      const groupIds = new Set(
                        ids
                          .map(
                            (id) =>
                              canvasElements.find((e) => e.id === id)?.groupId,
                          )
                          .filter(Boolean),
                      );
                      canvasElements
                        ...
                        .forEach((e) => {
                          if (onElementUpdate)
                            onElementUpdate(e.id, { groupId: undefined });
                        });
                      setCanvasCtxMenu(null);
                    }}
                  >
              ...
              <button
                ...
                onClick={() => {
                  if (onElementDelete)
                    onElementDelete(ids.length === 1 ? ids[0] : [...ids]);
                  setCanvasSelectedIds([]);
                  setCanvasCtxMenu(null);
                }}
              >
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(rgb(39, 39, 42) 1px, transparent 1px)",
      ...
      <div className="flex items-stretch min-h-0 min-w-0 w-full h-full justify-center gap-0">
        ...
        <div className="flex-1 flex items-center justify-center min-h-0 min-w-0">
          ...
          <div
            ...
            className={`relative shrink-0 overflow-hidden transition-all duration-300 ${mediaType === "image" && imageUrl ? "" : "border border-white/20 rounded-xl"}`}
            style={{
          ...
          >
              ...
              style={{
                perspective:
                  mediaType === "image" && imageTransform && apply3DToBackground
                    ? `${imageTransform.perspective || 600}px`
                    : "none",
            ...
            >
                  ...
                  <div
                    ...
                    style={{
                      ...
                      ...(shouldShowCustomColor && backgroundColorCss
                        ? backgroundColorCss.startsWith("#") ||
                          backgroundColorCss.startsWith("rgb")
              ...
              >
                ...
                {mediaType === "image" && apply3DToBackground && (
                  <div
                    ...
                    style={{ zIndex: 0, transform: "translateZ(-1px)" }}
                  >
                    {" "}
                    <div
                      className="absolute transition-all duration-200"
                ...
                <div
                  ...
                  style={{
                    ...
                    willChange:
                      zoomTransform.perspective > 0 ? "transform" : "auto",
                ...
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      style={{
                        transform:
                          ...
                          imageTransform &&
                          !apply3DToBackground
                            ? `                                                        translate(${videoTransform.translateX}%, ${videoTransform.translateY}%)                                                        rotate(${videoTransform.rotation}deg)                                                        rotateX(${imageTransform.rotateX}deg)                                                        rotateY(${imageTransform.rotateY}deg)                                                        rotateZ(${imageTransform.rotateZ}deg)                                                        scale(${imageTransform.scale * imageZoomScale})                                                        translateY(${imageTransform.translateY}%)                                                      `
                            : `translate(${videoTransform.translateX}%, ${videoTransform.translateY}%) rotate(${videoTransform.rotation}deg)`,
                        cursor: isDraggingVideo
                          ? "move"
                          : isVideoHovered && hasMedia
                            ? "move"
                            : "default",
                        transition:
                          mediaType === "image" &&
                      ...
                      onMouseDown={(e) => {
                        ...
                        if (onElementSelect) onElementSelect(null);
                        setVideoHoverCorner(
                          getNearestCorner(e, videoTransform.rotation),
                        );
                        ...
                        {isVideoSelected &&
                          ...
                          !isDraggingRotation && (
                            ...
                            >
                              ...
                              <div
                                style={{
                                  transform: `scale(${mediaType === "image" && imageTransform && !apply3DToBackground ? 1 / (imageTransform.scale * imageZoomScale) : 1})`,
                                  transformOrigin: "center center",
                                }}
                              >
                                {" "}
                                <RotationHandleIcon
                                  corner={videoHoverCorner}
                                  color="#e5e7eb"
                                />{" "}
                              </div>{" "}
                            </div>
                          )}{" "}
                        {(isVideoSelected || isVideoHovered) &&
                          hasMedia &&
                          !isDraggingRotation && (
                            <div
                              className={`absolute -inset-px border pointer-events-none z-10 opacity-80 ${isVideoSelected ? "border-blue-500" : "border-white"}`}
                              style={{
                                borderRadius: `${roundedCorners + 1}px`,
                              }}
                            />
                          )}{" "}
                        <div
                    ...
                    >
                      {" "}
                      <EditorHoverTooltip
                        show={isVideoHovered && mediaType === "image"}
                      />{" "}
                    </div>{" "}
                  </div>{" "}
                </div>{" "}
                ...
                {activeSpotlightFragment && (
                  <div
                    ...
                    style={{
                      ...
                      backdropFilter: activeSpotlightFragment.blur
                        ? `blur(${activeSpotlightFragment.blur}px)`
                        : undefined,
                    }}
                  >
                    {" "}
                ...
                {shouldShowSpotlight && cursorFrame && (
                  <div
                    ...
                    style={{
                      ...
                      backdropFilter: extendedCursorConfig.spotlightBlur
                        ? `blur(${extendedCursorConfig.spotlightBlur}px)`
                        : undefined,
                    }}
                  />
                )}{" "}
                      ...
                      cursorFrame.clicking && (
                        <div
                          ...
                          style={{
                            ...
                            backgroundColor:
                              cursorConfig.clickEffect === "ripple"
                                ? cursorConfig.clickEffectColor
                                : "transparent",
                            border:
                              cursorConfig.clickEffect === "ring"
                                ? `2px solid ${cursorConfig.clickEffectColor}`
                                : undefined,
                            opacity: 0.45,
                          }}
                        />
              ...
              {isDraggingVideo &&
                (mockupAlignmentGuides.vertical.length > 0 ||
                  mockupAlignmentGuides.horizontal.length > 0) && (
                  <>
                    {" "}
                  ...
                  <div
                    ...
                    onPointerMove={(e) => {
                      ...
                      const nextX = Math.min(
                        1,
                        Math.max(0, drag.initialX + dx),
                      );
                      const nextY = Math.min(
                        1,
                        Math.max(0, drag.initialY + dy),
                      );
                      onCameraConfigChange({
          ...
          <LayersPanel
            ...
            onReorder={(orderedIds) => {
              orderedIds.forEach((id, pos) => {
                if (onElementUpdate)
                  onElementUpdate(id, { zIndex: VIDEO_Z_INDEX + 500 - pos });
              });
            }}
            ...
            onUngroup={(ids) => {
              const groupIds = new Set(
                ids
                  .map((id) => canvasElements.find((e) => e.id === id)?.groupId)
                  .filter(Boolean),
              );
              canvasElements
                .filter((e) => e.groupId && groupIds.has(e.groupId))
            ...
            isVideoLayerSelected={
              selectedElementId === null && canvasSelectedIds.length === 0
            }
            onVideoLayerSelect={() => {
