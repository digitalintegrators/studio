...
import {
  ...
  MIN_ZOOM,
  ZOOM_STEP,
  type EffectInsertMode,
  type PlayerControlsProps,
} from "@/types/player-control.types";
...
export function PlayerControls({
  ...
  return (
    <div
      className="relative h-13 shrink-0 border-t border-white/10 flex items-center justify-between px-5 bg-[#0D0D11]"
      role="toolbar"
    ...
    >
      ...
      <div className="flex items-center gap-3 text-xs">
        <TooltipAction label={fullscreenLabel}>
          ...
          >
            <Icon
              icon={
                isFullscreen ? "typcn:arrow-minimise" : "typcn:arrow-maximise"
              }
              width="17"
              aria-hidden="true"
            />
        ...
        >
          ...
          <span
            className="text-[10px] font-mono text-zinc-500 min-w-5"
            aria-live="polite"
            aria-atomic="true"
          >
      ...
      <div
        ...
        className="absolute left-1/2 top-1/2 z-20 hidden -translate-x-[310px] -translate-y-1/2 md:flex items-center overflow-hidden rounded-full border border-white/10 bg-black/50 p-0.5 shadow-[0_8px_28px_rgba(0,0,0,0.28)] backdrop-blur-xl"
        role="group"
      ...
      <div
        className="absolute left-1/2 top-1/2 z-20 flex -translate-x-1/2 -translate-y-1/2 items-center gap-4"
        role="group"
        aria-label={t("transport.group")}
      >
        <span
          className="text-[11px] font-mono text-zinc-500"
          aria-label={t("transport.currentTime", {
        ...
        <span
          className="text-[11px] font-mono text-zinc-500"
          aria-label={t("transport.totalTime", {
            time: formatTime(videoDuration),
          })}
        >
