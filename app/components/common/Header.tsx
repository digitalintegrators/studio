...
export default function Header() {
  ...
  const navText = navCopy[locale as "es" | "en"] ?? navCopy.es;
  ...
  const getButtonIcon = () => {
    ...
    if (isRecording) {
      return (
        <span
          className="h-2.5 w-2.5 animate-pulse rounded-full bg-red-200 shadow-[0_0_18px_rgba(254,202,202,0.75)]"
          aria-hidden="true"
        />
      );
    }
      ...
      >
        <div className="mx-auto max-w-6xl px-3 sm:px-6">
          ...
          >
            ...
            <nav
              className="relative z-10 hidden items-center gap-1 rounded-full border border-white/10 bg-black/20 p-1 text-xs font-bold text-white/55 md:flex"
            ...
            <div className="relative z-10 flex items-center gap-2">
              ...
              <Button
                ...
                className={cn(
                  ...
                  isRecording &&
                    "border-red-300/45 bg-red-500/18 text-red-100 hover:bg-red-500/24",
                )}
              >
                {getButtonIcon()}
                <span>
        ...
        {showMobileAlert && (
          <div className="absolute left-1/2 top-20 w-full max-w-xs -translate-x-1/2 px-4">
            <Alert
              variant="warning"
              className="border-yellow-500/40 bg-[#0A0A0A]/95 backdrop-blur-xl"
              role="alert"
            >
              <Icon
